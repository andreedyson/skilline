...
const About = () => {
  return (
    <section id="about" className="max-w-[2000px] mx-auto px-6 py-12 lg:px-20">
        <div className="flex flex-col items-center gap-6 mb-20 text-center">
            ...
            <div className="flex flex-col justify-between w-full gap-14 lg:flex-row">
                {customers.map((customer, index) => (
                    <div key={customer.id} className={`relative flex flex-col items-center justify-center p-12 bg-cover lg:w-[600px] lg:h-[400px] rounded-[20px] 
                    ${index === 0 ? 'bg-instructor' : 'bg-student' }`}>
                        <div className="absolute inset-0 bg-black rounded-[20px] opacity-30"/>
                        <div className="z-[1]">
                            <h3 className="mb-4 text-xl font-semibold text-white uppercase md:text-3xl">{customer.name}</h3>
                            <button className="w-[200px] h-[50px] text-white ring-2 ring-white rounded-full duration-200 ease-in-out md:w-[280px] hover:bg-white hover:text-main-orange">{customer.button}</button>
                        </div>
                    </div>  
                ))}
            </div>
